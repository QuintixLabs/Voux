/*
  src/routes/settings/general.js
...
function registerSettingsRoutes(app, deps) {
  ...
  app.get('/api/settings', requireAdmin, (req, res) => {
    ...
    const visibleConfig = isOwner
      ? runtimeConfig
      : { ...runtimeConfig, autoBackup: undefined };
  ...
  app.post('/api/settings', requireAdmin, (req, res) => {
    ...
    if (auth?.type === 'admin') {
      const wantsRuntime =
        req.body &&
        ('privateMode' in req.body ||
          'showGuides' in req.body ||
          'allowedModes' in req.body ||
          'unlimitedThrottleSeconds' in req.body ||
          wantsAutoBackup);
      const wantsBranding =
        req.body &&
        ('homeTitle' in req.body ||
          'brandName' in req.body ||
          'theme' in req.body);
    ...
    if (typeof homeTitle === 'string') {
      patch.homeTitle = homeTitle.trim().slice(0, 120);
    }
  ...
  app.post('/api/backups/run', requireAdmin, async (req, res) => {
    ...
    try {
      ...
      return res.status(201).json({
        ok: true,
        backup: created.dbBackup,
        jsonBackup: created.jsonBackup || null
      });
    } catch (error) {
  ...
  app.post('/api/api-keys', requireAdmin, (req, res) => {
    ...
    try {
      ...
      const message =
        error.message === 'counters_required'
